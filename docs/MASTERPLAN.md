...
| E2 画廊 | 视频缩略/播/时长(**GA-1/2/7 ✅**);筛选(**GA-3 ✅**);存角色(**GA-4 ✅**——均 2026-08-05 Polish Wave 1);发送画布(GA-5,D-M4-2);删除(GA-6,D-M4-3,文件+DB 同步收敛) | GA-1~7 |
...
| E5 角色进阶 | 视频生成自动带角色图(**CH-1 注入链路 ✅ 2026-08-05 Polish Wave 1**——用户可见入口已随 CH-2 落地);video inspector 角色区(**CH-2 ✅ #230**);角色库管理页 | CH-1~3 |
...
| D-M4-1 脚本拆分 ✅ **已拍 A(2026-08-08)** | ~~SB-1/SB-2~~ 均已落地,E1 链贯通 | ~~SB-1 开工前~~ 已拍板 |
| D-M4-2~8（其余七项）✅ **已拍,全取 A 档(2026-08-21)** | ~~GA-5/6、AG-4/5、EX-2、CH-3~~ Wave 3 全解锁 | ~~M4 对应 Wave 开工前~~ 已拍板 |
...
- **D-M4-1 脚本拆分**:✅ **已拍 A 规则拆分**(2026-08-08;SB-1 落地
  `storyboard/util/script_splitter.dart`)。理由:纯规则零 LLM——没有 API key 也能用、
  行为可断言可单测不花钱;LLM 辅助质量不可断言且成本 ≥L,留作 AG-5 chat 模板后的可选增强
  (即 C 档,不废弃只延后)。**解锁 SB-1 → SB-2**;两卡均已落地,「粘贴脚本→分镜链→
  出图/出视频→序列预览→按叙事序导出」E1 整条链贯通。
- **D-M4-2 画廊→画布形态**:✅ **已拍 A 菜单动作**(2026-08-21)。一张 S 卡打通功能;
  真拖拽(B)是纯体验增强,后补不返工。**解锁 GA-5**。
- **D-M4-3 画廊删除语义**:✅ **已拍 A 永久删除行+文件**(2026-08-21)。与 LB-13 孤儿 GC
  语义一致,不留「隐藏但占盘」第三态;须带二次确认垫层。**解锁 GA-6**。
- **D-M4-4 聚合器生效时机**:✅ **已拍 A 编辑+重启先做**(2026-08-21)。运行时 add-only(B)
  为 L 级且动 JobQueue 调度预检,待真实需求再上(B 不废弃,顺延)。**解锁 AG-4**。
- **D-M4-5 第二协议模板**:✅ **已拍 A openai-chat-image**(2026-08-21)。chat 格式生态
  覆盖面最大;前置「模板→适配器分派表」(~20 行)先落。**解锁 AG-5**。
- **D-M4-6 导出转码档位**:✅ **已拍 A 单开关 1080p/30fps/H.264/丢音频**(2026-08-21)。
  异构拼接最小可用解;参数面板(C)明确不做。**解锁 EX-2**。
- **D-M4-7 项目复制取舍**:✅ **已拍 A 不复制 jobs/batch/exports**(2026-08-21)。复制的是
  创作资产不是运行历史;省 M 级工作量。
- **D-M4-8 角色库入口**:✅ **已拍 A 独立整屏仿 Gallery**(2026-08-21)。复用 Gallery
  路由/网格骨架,成本最低且不污染设置页。**解锁 CH-3**。
