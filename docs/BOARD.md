...
| **浮层 Esc 不可用是刻意的，不是遗漏**（shell-tab-navigation D11）：加 Esc 关浮层要在外壳层新增 `CallbackShortcuts`，这会与设置内部 `_EditorDialog` 的默认 `DismissIntent` 产生真实冲突——下一个人如果当 bug 顺手"修"掉这个空缺，后果是**在设置里按 Esc 会连编辑框带浮层一起关掉**（两层一起退出，用户以为只是关了一个输入框）。当前关闭浮层的途径是工具条返回键 / 点任一标签 / ⌘K 三条,已够用。补做时必须同时补一条"`_EditorDialog` 打开时 Esc 先关对话框、不关浮层"的竞争用例,否则会重犯这个回归 | 🅿️ | shell-tab-navigation 设计阶段拍板（D11），见 `lib/features/shell/README.md`「刻意不做」一节 |
