...
| 功能 | 状态 | 备注 |
|---|---|---|
| 分镜 / Shot 节点（脚本→分镜→序列） | 🔵 | Shot 编辑器落地（`3ba33d3`）；**首切片完成**：「用本镜备注生成图像」（shot_notes→image config 节点+narrative 边）+ FAB/空态 shot 创建入口 + addNode typeConfig 透传。**SB-5 链排序 util 落地**（`canvas/util/narrative_order.dart`——E1/E4 公共地基,同时解锁 SB-6 序列预览与 EX-1′ 导出叙事序）。**SB-3 落地**：shot 面板预期时长/预期运镜（意图口径,不按 provider 能力钳制;运镜文案抽 `util/camera_labels.dart` 双面板共享）。**SB-6 序列预览落地**（`features/storyboard`：顶栏「序列预览」→ 按叙事链把整条分镜从头播一遍;图片按时长、视频按真实进度推进,无产物镜显 notes 占位同计时）。**SB-4 落地**（shot→video 直达,镜头级参数一并带过去）。**D-M4-1 拍板 A 规则拆分**（2026-08-08）→ **SB-1 落地**（`storyboard/util/script_splitter.dart` 纯规则零 LLM,无 API key 也能用）。**SB-2 落地**（脚本导入对话框 + 批量建链:粘贴框/策略切换/实时预览 → `ScriptImportController` 单事务落 N 个 shot + N-1 条 narrative 边,失败整体回滚零残留;第一镜落视口中心,后续每 260 排开;入口=FAB 菜单「导入脚本…」+ 空态 CTA）——**「粘贴脚本→分镜链→出图/出视频→序列预览→按叙事序导出」E1 整条链自此贯通**。后续：LLM 辅助拆分（D-M4-1 的 C 档,不废弃只延后,待 AG-5 chat 模板） |
| 视频导出 / 拼接 | 🔵 | **服务层落地**：`VideoExportService.concat`（项目相对路径进出）→ `FfmpegVideoExportService`（concat demuxer 流拷贝 `-c copy`，不转码，要求输入同编码参数）+ `FfmpegLocator`（INKFRAME_FFMPEG env → PATH 探测，命中缓存）+ 最小 `ProcessRunner` 抽象（fake 可注入）；`FileResolverService.resolveInProject`（项目根边界安全校验，导出落 `exports/`）。**不打包 ffmpeg 二进制**（体积/许可评估延后）：系统有 ffmpeg 才真拼接，没有 → `LocalIOError(reason=ffmpeg_not_found)`。真 ffmpeg 集成测 `@Tags(['ffmpeg'])` TEST_FFMPEG=1 门控。**UI 入口落地**：`features/export`——画布顶栏「导出视频」按钮（无 video result 时禁用+说明 tooltip）→ 导出对话框（按 position.x 升序默认全选、复选/上下移手动排序、输出名本地预校验同 `_assertPlainFileName` 规则、busy 态 indeterminate、成功 snackbar+复制绝对路径、ffmpeg_not_found 专门文案 `exportVideoFfmpegMissing`）；`ExportController` 承担画布相对→项目相对路径换算（补 `canvases/<canvasId>/` 前缀）。**EX-1′ 落地**：默认序改 narrative 链序（经 `export/util/export_order.dart` + `canvas/util/node_artifacts.dart`，无链退回 position.x）+ 行首 16:9 缩略图。后续：转码/分辨率归一（EX-2，D-M4-6 待拍板）、打包二进制评估 |
...
| canvasViewportSizeProvider family + keepAlive → 每个开过的 canvasId 永久留一个 entry；后续让缩放路径改 watch 后去掉 keepAlive | 🅿️ | 持久标签外壳重构（shell-tab-navigation Task 2）引入：family 化本身是保活前必须的隔离修复,但 keepAlive 与 family 组合意味着「与 transform 对称」的旧注释在 dispose 语义上不再成立——transform 走 onDispose 释放 ChangeNotifier,viewport size 走 keepAlive 永不释放;待缩放路径改 watch 后可去掉 keepAlive,回到随画布 autoDispose |
| **浮层 Esc 分层**（shell-tab-navigation D11，原「刻意不做」）：加 Esc 关浮层曾被判定会与设置内部 `_EditorDialog` 的默认 `DismissIntent` 冲突——在设置里按 Esc 连编辑框带浮层一起关掉 | ✅ | **已解决**（视觉重做设置浮层接线，#235 commit `54ef026`）：Esc 的 `CallbackShortcuts` 不挂外壳层，挂在设置浮层**自己持焦**的节点上；`showDialog` 弹出的编辑框在 Navigator 的另一条路由上、焦点不在浮层子树下，Esc 由对话框自己的 `DismissIntent` 吃掉，到不了浮层——编辑框开着只关编辑框，关掉后再按一次才关浮层。竞争用例就是当年要求补的那条：`test/features/settings/settings_overlay_test.dart`「Esc 分层 ①」；「Esc 分层 ②」断言关浮层后标签与画布状态不变 |
| 「关闭当前画布」待补卡：`ShellState` 没有任何能清 `canvasId` 的公共动词（`resetSession()` 全清除外）——老代码里"清 canvasId"的真实意图都是"回 Studio",本 PR 的替代是 `goTab(studio)`,画布本身并未从保活槽卸载 | 🅿️ | shell-tab-navigation 设计阶段拍板：真需要"关闭画布"菜单时另加具名迁移 + 一条"调用后保活被销毁"的显式断言,不在本 PR 范围 |
| 画廊脏标记不分项目：`JobState` 的六个分支只带 `canvasId`,不带 `projectId`,`galleryDirtyProvider` 因此是全局脏标记——A 项目生成完成、切到 B 项目画廊也会触发一次刷新（走 `skipLoadingOnRefresh` 就地换数据,用户无感,但语义上不精确） | 🅿️ | shell-tab-navigation 执行期发现,裁定推后：要做到按项目精确必须先给 `JobState`/registry 补 `projectId`,超出本 PR 范围 |
| `canExportVideo` 的半个孤儿判据：`videoNodes.first.projectId != null` 这条子判据在 T10 把 projectId 改走外壳态（`ShellState.project`）之后,只剩"启用条件"这一半还有意义（另一半"取 projectId 供 `_open` 使用"已被外壳态取代） | 🅿️ | shell-tab-navigation T10 明令判据原样搬运不重写（动它会改变按钮禁用语义）,留待单独裁决 |
| 导出按钮 label 与 tooltip 是同一句：`export_tab.dart` 两处都用 `exportVideoTooltip`,按钮面显示「导出视频」、悬停 tooltip 也是「导出视频」,零信息量;序列侧的入口已经做对（label 用短标题、整句留给 tooltip） | 🅿️ | shell-tab-navigation 执行期发现;正解是给导出加一句真 tooltip 文案（说明按钮为何禁用/导出内容是什么）,不是改 label,随下一个动导出入口的窗口顺手补 |
| `ShellState.openCanvas` 的 `withProject` 可考虑改成必传：当前 `project == null` 时静默沿用旧值的分支在生产代码里不可达（三个调用点全部带 `withProject`）,但若日后新增一个不带 `withProject` 的调用点,按钮会静默变成哑键、且没有任何红测能抓到 | 🅿️ | shell-tab-navigation 执行期发现,记账观察;真出现第二个不带参调用点时再决定是否收紧签名 |
| 设置页九个既有 section（除 T12 新增的 `StartupSection` 外）无「存在性断言」测试覆盖 | 🅿️ | shell-tab-navigation T12 范围裁定：只补本 PR 新增部分,九个既有 section 的测试缺口是既有状态,批量补属于范围蔓延,另开卡处理 |
| `ShellState.setProject` / `ShellNavigator.setProject` 是**有测试的死 API**：`lib/` 下只有定义，全部消费点都在测试里（spec §5.1 列了它，但没有任何生产路径调用）。切项目今天走 `openGallery(ProjectRef)` 或 `openCanvas(id, withProject:)` 两条具名迁移，`setProject` 这条"只换项目、不动标签"的语义没有对应的 UI 动作 | 🅿️ | shell-tab-navigation 最终评审（R85–R90 轮）发现。裁决：不在本 PR 删——它是 7 个具名迁移之一、有表驱动单测钉死语义，删它要连带动 `shell_state_test.dart`；等真出现"只换项目不换标签"的交互（如项目切换器）时再决定是接上还是退役 |
| `ink_shell_tab_bar.dart` 的 chip 是 `GestureDetector` + `Semantics`，**不可键盘聚焦、不可回车激活**——只能鼠标点。与仓库既有 `InkGhostButton` 同款，不是本 PR 的退步 | 🅿️ | shell-tab-navigation 最终评审（R85–R90 轮）发现。标签条是全应用最高频交互，做键盘导航时这里是第一站；正解是把 chip 换成 `InkWell`/`FocusableActionDetector`（带 focus ring token），并连带把 `InkGhostButton` 一起收口，避免只修一处留下两套可达性语义 |
| 导出的**门控**仍从节点数据摸 projectId,与已迁走的**动作**错位:`command_actions.dart:76-77` 与 `tab_availability.dart:26` 都是 `videoNodes.isNotEmpty && videoNodes.first.projectId != null`,而 R86 已把两处**动作**改成 `ShellState.project?.id`。错位方向因此被**反转**而非消除——外壳里项目上下文完好、但"原序第一个"可导出 video 的 `project_id` 为空时,`canExport` 为假 ⇒ ⌘K 里根本不出「Export video」、导出标签按钮禁用,**而此时导出本来完全能跑**,用户看到的是"功能莫名其妙没了"且无任何提示 | 🅿️ | shell-tab-navigation 最终复评发现(非本分支退步:spec §8.2 明令"可用性判据原样搬运不重写",该判据是从已删除的 `canvas_top_chrome.dart` 逐字搬来)。正解=`videoNodes.isNotEmpty && shell.project != null`,或直接去掉 projectId 那一项。**刻意不在合并前动**:改门控会牵动 `shell_tabs_empty_state_test.dart` 整批搬运来的禁用判据用例,属范围蔓延。**第 5/6 步做完整序列/交付面板时,门控判据必须改成读项目上下文(`ShellState.project`),不能沿用节点数据——否则新面板会原样继承这个错位**(设计侧 2026-09-22 指出) |
| Workspace v2 稿的检查器「镜头运动」组有四个字段没有数据模型：**景别 / 机位角度 / 运镜幅度 / 焦段**（稿上是四个下拉/滑杆）。S2b 接线时按「无后端不画」规则未画，检查器只画了运镜方式（`camera`，全量枚举）与片长 | 🅿️ | 视觉重做 B 路径 S2b（#235）用户拍板：先记债。要画就得给 shot / video 的 `type_config` 加键 + 各 provider 决定是否透传（多数 provider 没有这些参数，很可能只是 prompt 拼接），属于数据模型决策，不在视觉 PR 里做 |
| 稿的检查器「模型」组显示**模型名**（如 kling-v3 下的具体模型）与**fps**；`ProviderCapabilities` 只有 providerId / 分辨率 / 时长档，没有模型名与帧率元数据。S2b 按「provider 元数据有才只读显示，否则省略」拍板，两项都省略 | 🅿️ | 同上（#235）。补齐要在 `ProviderCapabilities` 加 `modelName` / `fps` 字段并让 15 个实现体填值；渲染队列「模型」列眼下显示的也是 providerId |
| 画廊筛选 / 选中集 provider 是**刻意的 keepAlive**（`galleryFilterProvider` / `gallerySelectionProvider`，按 projectId 分键），不是疏忽：每访问一个项目就多留一份状态直到进程退出——与画布那边「D3 跨画布隔离是刻意的」是反方向的取舍 | 🅿️ | 视觉重做画廊接线（#235，用户拍板 2026-09-23）：状态很小（四个可空枚举 + 一个 key 集合），留着换来「切标签 / 切项目再回来筛选与选中不丢」。改回 autoDispose 会让 `shell_gallery_state_retention_test` 的三条（筛选还在 / 控件一致 / 滚动保持）翻红——原先靠 GalleryScreen 顶层一条 watch 锚存活，切项目那一刻 watcher 断、筛选被回收。项目数上百时再考虑 LRU 上限（例如保留最近 8 个） |
| ⌘K「镜头」组只搜**当前画布里已加载**的节点（组标题写明「镜头 · 当前画布」）；跨画布 / 跨项目搜镜头没有做——需要单独的查询（nodes 表按 label / prompt 的项目级检索），不能靠把所有画布的 controller 都 watch 起来凑 | 🅿️ | 视觉重做命令面板接线（#235，用户拍板 2026-09-23）：「不为搜索做全量加载」。同理「设置项」组没有索引，先不显示。「产物」组用的是画廊那份项目级聚合（`galleryControllerProvider`），有项目上下文才出现 |
