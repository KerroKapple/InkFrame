...
工作区里曾存在两份**从未提交到任何分支**的未跟踪文件：

- `docs/superpowers/specs/2026-09-14-shell-overlay-navigation.md`
- `docs/superpowers/plans/2026-09-14-shell-overlay-navigation.md`

它们描述的是 `base + overlay` 导航模型（画廊/设置/示例以全屏覆盖层挂在 `Offstage` 画布之上，无标签条）。该文件内记录的「2026-09-14 用户拍板」无法核实，项目所有者亦无此决策记录。**本规格取代之，那份模型作废，不得引用其形态结论。**

其中列出的「共同地基」仍然成立并被本规格吸收：路由直写收口到单一写入口、删除 `_openSettings` 的跨清写、画布保活、备份还原重置、启动恢复守卫改判。
