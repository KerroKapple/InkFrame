...
// 另有自定义 Provider（custom:<id>，PROVIDER-API §13）：来自 customProviderSourceProvider
// （main 启动期一次性加载 custom_providers.json，会话内不变），能力由协议模板派生。
//
// keySource 统一走 SecureStorage；DashScope 家族经 SecureStorageKeys.scopeOf
// 折叠到同一把 Key（custom:<id> 走 scopeOf 回退，即 providerId 本身）。
//
...
// - 本 provider 不可 invalidate：JobQueue 连锁重建会把运行中任务打成 cancelled
...
import '../models/provider_protocol_template.dart';
...
final providerRegistryProvider = Provider<ProviderRegistry>((ref) {
  ...
  final List<CustomProviderConfig> customConfigs =
      ref.watch(customProviderSourceProvider).configs;
  ...
  return CachingProviderRegistry(<String, ProviderFactory>{
    ...
    // 自定义 Provider（PROVIDER-API §13）：文件服务已保证 id 唯一且不与内置冲突；
    // limiter/实例同享 CachingProviderRegistry 的"每 providerId 只建一次"不变量。
    for (final CustomProviderConfig config in customConfigs)
      config.providerId: () => OpenAICompatibleImageProvider(
            config: config,
            keySource: keyFor(config.providerId),
            rateLimiter:
                limiterFor(deriveCustomProviderCapabilities(config)),
          ),
  });
});

/// 全部内置 Provider 的能力声明（编译期 const）。注册顺序即展示顺序。
...
/// UI 层下拉菜单数据源：内置 const 能力 + 自定义模板派生能力的**同步**合并——
/// 不实例化任何 Provider（不连带 Dio / RateLimiter，P1-x1 修复点）。
/// 必须保持同步可读（image inspector 在 initState 里 ref.read）；
/// custom 为空时原样返回 const 列表（零分配）。
final providerCapabilitiesListProvider = Provider<List<ProviderCapabilities>>(
  (ref) {
    final custom = ref.watch(customProviderSourceProvider).configs;
    if (custom.isEmpty) return kAllProviderCapabilities;
    return List.unmodifiable(<ProviderCapabilities>[
      ...kAllProviderCapabilities,
      for (final config in custom) deriveCustomProviderCapabilities(config),
    ]);
  },
