// ProcessRunner / ProcessStarter 抽象：外部进程执行的最小注入点。
//
// PgProcessRunner（lib/storage/pg_controller.dart）是 PG 专用的窄接口。
// run() = 一次性缓冲执行（当前消费方：ffmpeg 探测、URL 打开、目录 reveal）；
// start() = 流式通道（当前消费方：视频导出 EX-3、pg_dump/pg_restore 经
// runWithWatchdog——PGPASSWORD 注入走 start 的 [environment]）。
// 单测注入 fake 捕获命令行，生产用 SystemProcessRunner（同时实现两接口）。
//
// 失败语义：可执行文件不存在/不可启动时抛 [ProcessException]（dart:io 原生），
// 由调用方映射为 InkError。
//
// [environment]：附加环境变量（并入父进程环境，不替换）。
import 'dart:io';
