...
void main() {
  ...
  test('ME-31：失败迁移整体回滚——版本号与半截 DDL 都不落库', () async {
    try {
      final h = req();
      // 故意失败的「链末版+1」迁移：先建表再撞语法错误；事务内两者都应回滚。
      // 版本号动态取末版+1——若与已应用版本重号会被 runner 静默跳过，测试失效。
      final tail = kAppMigrations.last.version;
      ...
      expect(v.first[0], tail, reason: '失败迁移不得推进版本号');
